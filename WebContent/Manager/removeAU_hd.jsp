...
<%
	...
	String text1 = "", text2 = "";
	Adminr radminr = new Adminr();
	User ruser = new User();
	text1 = request.getParameter("text1");
	text2 = request.getParameter("text2");
	if("remove_a".equals(action))		
   	{
		if (text1.equals("")) {
			out.println("<script>alert('用户名不能为空');</script>");
		} else if (!(radminr.isExist(text1))) {
			out.println("<script>alert('此管理员不存在');</script>");
		} else {
			radminr.deleteadmin(text1);
			out.println("<script>alert('删除成功')</script>");
		}
	} else if("remove_u".equals(action)) {
		if (text2.equals("")) {
			out.println("<script>alert('名称不能为空');</script>");
		} else if (!(ruser.isExist(text2))) {
			out.println("<script>alert('此专辑不存在');</script>");
		} else {
			radminr.delete(text2);
			out.println("<script>alert('删除成功')</script>");
		}
	}
%>
    ...
    <div class="h_center1">删除用户</div>
    <div class="h_center">
    <form id="form1" name="form1" method="post" action="removeAU_hd.jsp?action=remove_a">
    	<table class="l_table">
     		<tr>
	   		<td width="25%">请输入管理员名称</td>
   	...
   	<form id="form2" name="form2" method="post" action="removeAU_hd.jsp?action=remove_u">
    	<table class="l_table">
     		<tr>
	   		<td width="25%">请输入用户名称</td>
